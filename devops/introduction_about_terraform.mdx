...
<Columns cols="2">
  <Card title="Multi-Cloud Support" href="#" icon="cloud" horizontal="false">
    Supports AWS, Azure, GCP, DigitalOcean, and 100+ other providers.
  </Card>

  <Card title="Orchestration" href="#" icon="layers" horizontal="false">
    Handles full infrastructure orchestration, not just configuration management.
  </Card>

  <Card title="Immutable Infrastructure" href="#" icon="refresh-cw" horizontal="false">
    Configuration changes are applied smoothly with version-controlled state.
  </Card>

  <Card title="HCL Language" href="#" icon="code" horizontal="false">
    Uses HashiCorp Configuration Language - easy to read and write.
  </Card>

  <Card title="Portable" href="#" icon="move" horizontal="false">
    Easily portable configurations across different cloud providers.
  </Card>

  <Card title="Client-Only" href="#" icon="monitor" horizontal="false">
...
<CodeGroup show-lines="true" tabs={["Debian","CentOS"]}>
  ```bash Debian
...
1. **Terraform Configuration** - You define what needs to be created or provisioned in `.tf` files
2. **State File** - Terraform keeps an up-to-date state of the current infrastructure
...
sequenceDiagram
    ...
    User->>Config: Write/Modify .tf files
    User->>Core: terraform init
    Core->>Provider: Download required providers
    User->>Core: terraform plan
    Core->>State: Read current state
    Core->>Config: Read desired state
    Core-->>User: Show execution plan
    User->>Core: terraform apply
    Core->>Provider: Create/Update/Delete resources
    Provider->>Infra: Provision resources
    Provider-->>Core: Resource details
...
<ParamField path="provider" param-type="string" required="true" deprecated="false">
  Plugin that interacts with cloud provider APIs (AWS, Azure, GCP, etc.) to manage resources.
</ParamField>

<ParamField path="resource" param-type="string" required="true" deprecated="false">
  Block defining one or more infrastructure objects (EC2 instances, VPCs, storage buckets).
</ParamField>

<ParamField path="data_source" param-type="string" required="false" deprecated="false">
  Read-only reference to existing infrastructure objects managed outside Terraform.
</ParamField>

<ParamField path="state" param-type="file" required="true" deprecated="false">
  JSON file (terraform.tfstate) storing cached information about managed infrastructure.
</ParamField>

<ParamField path="module" param-type="string" required="false" deprecated="false">
  Directory containing reusable Terraform templates and configurations.
</ParamField>

<ParamField path="variable" param-type="string" required="false" deprecated="false">
...
**A key point about count and for\_each is that, although we can assign
...
<Steps>
  <Step title="terraform init" icon="download" titleType="h3">
  ...
  <Step title="terraform plan" icon="file-text" titleType="h3">
  ...
  <Step title="terraform apply" icon="check" titleType="h3">
  ...
  <Step title="terraform destroy" icon="trash" titleType="h3">
