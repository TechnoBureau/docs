...
Production-ready client supporting both JDBC URL format and direct DSN format.
...
| AWS | ALB | 120s (custom) | 60s | Yes |
| AWS | NLB | 350s (**fixed**) | 180s | **No** |
| Azure | Standard LB | 240s (default) | 120s | Yes — 4 to 30 min |
| Azure | Standard LB | 600s (10 min) | 300s | Yes |
| Azure | Standard LB | 1800s (30 min) | 900s | Yes |
...
| AWS ALB 120s custom | 120s | 60s | 100s |
| AWS NLB (fixed) | 350s | 180s | 300s |
| Azure default | 240s | 120s | 120s |
| Azure 10 min | 600s | 300s | 540s |
| Azure 30 min | 1800s | 900s | 1620s |
...
```mermaid
flowchart TB
    A["🔵 App Query Timeout — 30s\nPrevents long-running queries"]
    B["🔵 Connection Pool Idle — 5 min\nRecycles stale pool connections"]
    C["🟠 NGINX proxy_timeout — 5 min\nCloses truly idle proxy connections"]
    D["🟢 TCP Keepalive — 60s idle · 10s retry · 9 probes\nDetects network-level failures · Total detection: 150s"]
    E["🔴 Cloud NAT Timeout — 240s–350s\nExternal constraint — keepalive must fire before this"]
    F["🟣 Database wait_timeout — 8h (MySQL)\nServer-side safety net"]

    A --> B --> C --> D --> E --> F
```

### Connection Failure — Root Cause Flow

```mermaid
sequenceDiagram
    participant APP as 🖥️ Application
    participant NGX as ⚙️ NGINX Proxy
    participant DB as 🗄️ Database

    APP->>NGX: Connect + Query
    NGX->>DB: Forward
    DB->>NGX: Response
    NGX->>APP: Forward

    Note over APP,DB: ⏳ 300s of silence — proxy_timeout expires

    Note over NGX: NGINX silently closes connection (no FIN sent)
    Note over APP: App still thinks connection is ALIVE

    APP->>NGX: Reuse connection — send query
    NGX-->>APP: ❌ connection reset by peer
```
...
```mermaid
flowchart TD
    START["Connection reset / timeout error?"]
    Q1{"ss shows\ntimer keepalive?"}
    Q2{"tcpdump sees\npackets on port 3306?"}
    Q3{"tshark shows\nkeep_alive rows?"}
    Q4{"Probe interval\nmatches sysctl?"}

    FIX_KA["SO_KEEPALIVE not set\n→ Add KeepAlive: -1 to dialer\nor tcpKeepAlive=true in JDBC URL"]
    FIX_IF["Wrong interface\n→ Check ip link or tcpdump -D"]
    FIX_CTX["pcap missing handshake\n→ Recapture without BPF filter:\nsudo tcpdump 'tcp port 3306'"]
    FIX_APP["App sends periodic queries\n→ Remove SELECT 1 heartbeat\nLet connection go fully idle"]
    OK["✅ Keepalive working\n→ Check LB timeout alignment\nand proxy_timeout values"]
