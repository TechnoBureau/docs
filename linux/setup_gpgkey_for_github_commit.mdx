...
<Steps>
  <Step title="Obtain GPG key file" icon="file" titleType="p">
    If you have the GPG key file (with a .asc or .gpg extension) from another source, make sure you have it available on your system.
  </Step>
  <Step title="Import GPG key" icon="download" titleType="p">
    Open your terminal and run the following command, replacing `<KEY-FILE>` with the path to your GPG key file:
