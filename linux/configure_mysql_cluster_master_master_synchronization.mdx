...
description: >-
  MySQL Cluster is a powerful feature in MySQL that allows for high availability
  and scalability by distributing the database across multiple servers. In this
  article, we will walk through the configuration steps for setting up a MySQL
  Cluster with master-master synchronization.
...
MySQL Server Identification:

<CodeGroup default-tab="0" show-lines="true" tabs={["MySQL1","MySQL2"]}>
  ```text MySQL1
...
<CodeGroup show-lines="true" tabs={["MySQL1","MySQL2"]}>
  ```bash MySQL1
  [mysqld]
