...
```mermaid
graph TD;
    client[Client] -->|HTTP/HTTPS| cloudLB[Cloud Load Balancer];
    cloudLB -->|TCP| connectionB[Connection B - TCP];
    connectionB -->|TCP/HTTP| nginx[NGINX Ingress Controller];
    nginx -->|Backend| backend[Backend Service / Database];
...
<Steps>
  <Step title="Configure ALB Idle Timeout via AWS CLI" icon="terminal" titleType="p">
    Retrieve the ALB ARN and update the idle timeout.
...
```mermaid
graph TD;
    NLB["AWS Network Load Balancer (NLB)"] -->|Idle Timeout| idle["350s (fixed)"];
    idle -->|Cannot be configured| noConfig["No annotation / CLI / Terraform"];
    noConfig -->|Requirement| keepalive["Set tcp_keepalive_time < 350s (e.g., 180s)"];
...
```mermaid
graph TD;
    start["Connection established at t=0"] --> idle["Application idle"];
    idle -->|t = tcp_keepalive_time e.g., 120s| probe1["Send KEEPALIVE probe #1"];
    probe1 --> wait1["Wait for ACK"];
    wait1 -->|ACK received| reset1["Reset timer"];
    wait1 -->|No ACK| intvl1["Wait tcp_keepalive_intvl. e.g., 15s"];
    intvl1 --> probe2["Send KEEPALIVE probe #2"];
    probe2 --> wait2["Wait for ACK"];
    wait2 -->|ACK received| reset2["Reset timer"];
    wait2 -->|No ACK| intvl2["Wait tcp_keepalive_intvl"];
    intvl2 --> probe3["Send probe #3"];
    probe3 --> continue["Continue…"];
    continue -->|After N failed probes| dead["Declare connection DEAD<br>Send RST"];
...
flowchart TB
    T0["t=0<br/>━━━━━━━━━━━━━━━━━━<br/>🟢 Connection Established<br/>Flow Entry Created<br/>Client:12345 ↔ Backend:80"]

    T120["t=120<br/>━━━━━━━━━━━━━━━━━━<br/>📤 Last Data Packet<br/>⏱️ Idle Timer STARTS<br/>Timeout Setting: 240s"]

    T200["t=200 (80 seconds idle)<br/>━━━━━━━━━━━━━━━━━━<br/>✅ TCP Keepalive Probe Arrives<br/>⏱️ Idle Timer RESETS to 0<br/>Flow Entry: ACTIVE"]

    T320["t=320 (120 seconds after last keepalive)<br/>━━━━━━━━━━━━━━━━━━<br/>✅ TCP Keepalive Probe Arrives<br/>⏱️ Idle Timer RESETS to 0<br/>Flow Entry: ACTIVE"]

    T440["t=440, t=560, t=680...<br/>━━━━━━━━━━━━━━━━━━<br/>✅ Keepalives Continue<br/>⏱️ Timer Keeps Resetting<br/>Flow Entry: ACTIVE"]

    RESULT["✅ RESULT<br/>━━━━━━━━━━━━━━━━━━<br/>🟢 Connection Stays ALIVE<br/>🟢 Flow Entry Maintained<br/>🟢 No Connection Errors<br/>🟢 Application Works Fine"]

    T0 --> T120 --> T200 --> T320 --> T440 --> RESULT
```
...
flowchart TB
    ...
    T200["t=200 (80 seconds idle)<br/>━━━━━━━━━━━━━━━━━━<br/>💤 No Keepalive Sent<br/>⏱️ Idle Timer: 80/240s<br/>Flow Entry: ACTIVE (but counting...)"]

    T300["t=300 (180 seconds idle)<br/>━━━━━━━━━━━━━━━━━━<br/>💤 Still No Packet<br/>⏱️ Idle Timer: 180/240s<br/>Flow Entry: ACTIVE (but counting...)"]

    T360["t=360 (240 seconds idle)<br/>━━━━━━━━━━━━━━━━━━<br/>⏰ IDLE TIMEOUT REACHED!<br/>🗑️ Flow Entry REMOVED<br/>Load Balancer Forgets Connection"]

    T400["t=400<br/>━━━━━━━━━━━━━━━━━━<br/>📤 Client Tries to Send Packet<br/>❌ Load Balancer: No Flow Entry Found<br/>❌ Connection Mapping Lost"]

    RESULT["❌ RESULT<br/>━━━━━━━━━━━━━━━━━━<br/>🔴 Load Balancer Sends TCP RST to Client<br/>🔴 Load Balancer Sends TCP RST to Backend<br/>🔴 Error: 'connection reset by peer'<br/>🔴 Application Must Reconnect"]

    T0 --> T120 --> T200 --> T300 --> T360 --> T400 --> RESULT
...
| Provider | LB Type  | LB Timeout       | keepalive_time | keepalive_intvl | Notes                    |
|----------|----------|------------------|----------------|-----------------|--------------------------|
| AWS      | ALB      | 60s (default)    | 30s            | 10s             | Configurable: 1-4000s    |
| AWS      | ALB      | 120s (custom)    | 60s            | 10s             | Custom configuration     |
| AWS      | NLB      | 350s (fixed)     | 180s           | 15s             | Fixed, cannot change     |
| Azure    | Standard | 240s (default)   | 120s           | 15s             | Default idle timeout     |
| Azure    | Standard | 600s (10m)       | 300s           | 20s             | Medium idle timeout      |
| Azure    | Standard | 1800s (30m)      | 900s           | 30s             | Maximum idle timeout     |
...
```
flowchart LR
  client[Client] --> http["HTTP/HTTPS Block"]
  http --> upstream["Upstream Block"]
  upstream --> server["Server Block"]
  server --> location["Location Block"]
  location --> nginx["NGINX Process"]

  classDef box
      client,http,upstream,server,location,nginx:::box
```
...
flowchart LR
    A[Application] --> B[NGINX<br/>Stream Proxy]
    B --> C[Cloud NAT]
    C --> D[Database]

    subgraph Timeout_Layers
        D1[Database Idle Timeout<br/>Varies by DB]
        D2[Cloud NAT Idle Timeout<br/>AWS: ~350s<br/>Azure: 240–1800s]
        D3[NGINX Stream Proxy Timeout]
        D4[No "Last Packet" from Idle Connection]
        D5[Connection Reset Errors]
    end
...
sequenceDiagram
    ...
    participant NGX as ⚙️ NGINX
    participant DB as 🗄️ Database

    Note over APP,DB: t=0 — Connection Established
    APP->>NGX: Connect
    NGX->>DB: Connect
    Note over APP,DB: ✅ TCP Connection Open

    Note over APP,DB: t=1 — Query Sent
    APP->>NGX: Send Query
    NGX->>DB: Forward Query

    Note over APP,DB: t=2 — Response Received
    DB->>NGX: Query Response
    NGX->>APP: Forward Response
    Note over APP,DB: ✅ Connection Now Idle at t=10

    Note over APP,DB: ⏳ Silence... 300 seconds pass...

    Note over NGX: t=310 — NGINX proxy_timeout expires ❌
    Note over NGX: ❌ NGINX silently closes connection
    Note over NGX: ❌ No FIN sent to Application
    Note over NGX: ❌ No RST sent to Application
    Note over APP: ❌ Application still thinks connection is ALIVE

    Note over APP,DB: t=320 — Application tries to reuse connection
    APP->>NGX: Send Query ❌
    NGX-->>APP: Connection Reset / Refused
    Note over APP: ❌ Error: connection reset by peer
    Note over APP: ❌ Error: no last packet sent
```
